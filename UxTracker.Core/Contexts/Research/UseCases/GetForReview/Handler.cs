...
public class Handler(IRepository repository) : IRequestHandler<Request, Response>
{
    public async Task<Response> Handle(Request request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            project = await repository.GetProjectsByIdAsync(request.UserId, request.ProjectId, cancellationToken);
        ...
        try
        {
            accepted = await repository.IsTermAcceptedAsync(request.UserId, request.ProjectId, cancellationToken);
        }
        catch
        {
