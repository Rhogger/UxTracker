...
public static class Specification
{
    public static Contract<Notification> Ensure(Request request)
        => new Contract<Notification>()
            ...
            .IsLowerOrEqualsThan(request.Description.Length, 1000, "Description", "A descrição deve conter no máximo 1000 caracteres")
            .IsGreaterOrEqualsThan(request.Description.Length, 20, "Description", "A descrição deve conter pelo menos 20 caracteres")
            // .IsLowerOrEqualsThan(request.StartDate, DateTime.UtcNow.AddDays(-1), "StartDate", "A data inicial deve anteceder 24 horas a data final")
